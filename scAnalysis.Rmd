...
```{r data_cleaning_filtering, dev=c('png','svg'), echo=F}
total_cells = all_data %>%
  group_by(treatment) %>%
  summarise(cell_count = length(unique(cell_id)),
            read_total = sum(count))

ggplot(total_cells,aes(x=treatment,y=cell_count)) + geom_bar(stat="identity")

ggplot(total_cells,aes(x=treatment,y=read_total)) + geom_bar(stat="identity")
```
